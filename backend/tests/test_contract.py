...
driving `klartex.server.app` — the very app the render image runs. A
TestClient is a synchronous httpx.Client subclass with its own transport,
and accepts absolute URLs whatever the hostname says.

What that buys over mocking either side is the part no unit test can see:
that the payload the backend builds really parses as the core's request
model, and that the error bodies the core produces really survive the
backend's validation and reach the client unchanged. Without xelatex the
validation and input paths still run; the two tests that need a real PDF
skip themselves, as they do in CI.
"""
...
from klartex.server.app import app as render_app
...
def wired_to_render(monkeypatch):
    """Send the backend's render calls to the core's render app in-process."""
...
def test_a_bad_page_template_object_is_validated_by_the_core(registry):
    """The slot form is the core's contract, and the core still owns it."""
    r = post_blocks(
        [{"type": "heading", "text": "x"}],
