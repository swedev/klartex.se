...
- [x] Fas 2, steg 3: Endpoint-test för tomt `type` (`Block at body[i] is missing 'type'`) plus enhetstest på `_block_error_path()` utan match
...
- `Block at body[i] is missing 'type'` nås via `{"type": ""}` — tomma strängen passerar toppnivåschemat. Planens tabell och Fas 2 steg 3 är rättade efter PR-granskningen; formen testas via endpointen.
