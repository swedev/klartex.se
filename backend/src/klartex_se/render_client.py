...
The render service owns the error shapes: it is the process that calls the
klartex core, so it is where the core's messages are interpreted. This
module's job is therefore narrow. It sends the request, hands back the PDF,
and translates everything else into a `RenderUpstreamError` the endpoint can
turn into an HTTP response:
...
def _client() -> httpx.Client:
    ...
    replace the transport — test_contract.py substitutes a TestClient that
    drives the render app in-process.
    """
...
def _upstream_detail(response: httpx.Response) -> dict | None:
    ...
    ...}}` — may reach the client. A pydantic 422 from the render service,
    an HTML error page or anything else falls through to the 502.
    """
    try:
