...
- [x] **Fas 1 (PR 2a): ikapp kärnan på den monolitiska imagen.** Kärnreleasen `klartex 0.17.0` bär både alias-borttagningen (swedev/klartex#80) och `klartex serve` + render-imagen (swedev/klartex#81), så blockeraren är löst.
  ...
  - Shimmen (Designbeslut 11): bundlens `page_template.tex.jinja` skickas som `header_source` och `data.page_template.footer` sätts till `null`. Dokumentinställningarna i `data.page_template` (`font`, `header_font`, `diff_style`, `page_numbers`, `first_page_header`) behålls; både `header` och `footer` som anropet skickar får ge vika — kärnan låter `header_source` ta header-sloten ovillkorligt.
